...
(function () {
  ...
  var CATEGORY_ICONS = {
    ...
    'arcade': '<svg viewBox="0 0 80 80" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="10" y="24" width="60" height="36" rx="8" fill="#fff" fill-opacity="0.08"/><circle cx="58" cy="36" r="4" fill="#fff" fill-opacity="0.3" stroke="none"/><circle cx="58" cy="48" r="4" fill="#fff" fill-opacity="0.2" stroke="none"/><line x1="22" y1="34" x2="34" y2="46"/><line x1="34" y1="34" x2="22" y2="46"/></svg>',
    'word': '<svg viewBox="0 0 80 80" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"><text x="40" y="52" text-anchor="middle" font-size="32" font-weight="bold" fill="#fff" fill-opacity="0.3" stroke="none">A</text><path d="M18 62 L44 62" stroke-width="2"/></svg>',
    'idle': '<svg viewBox="0 0 80 80" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"><circle cx="40" cy="44" r="18"/><path d="M40 44 L40 32"/><path d="M40 44 L50 38"/><path d="M40 14 L40 20 M66 44 L60 44 M40 74 L40 68 M14 44 L20 44"/></svg>',
  };
  ...
  /* Search routing: non-empty queries are swallowed here and filtered by
     the instant fuzzy overlay (applyFuzzySearch) instead of the bundle's
  ...
  /* --- Favorites toggle: the bundle owns the star buttons + persistence
     (localStorage `unblockmath_favorites`, `.game-card__fav.active`).
     This layer adds only the missing Favorites filter above the grid. --- */
  var favOnly = false;
  var favToggleBtn = null;

  function isBundleFav(card) {
    var btn = card.querySelector ? card.querySelector('.game-card__fav') : null;
  ...
  function applyFavFilter() {
    $$('.bento-grid .game-card').forEach(function (card) {
      if (!favOnly || isBundleFav(card)) card.removeAttribute('data-ux-fav-hidden');
      else card.setAttribute('data-ux-fav-hidden', '1');
    });
    if (favToggleBtn) favToggleBtn.setAttribute('aria-pressed', favOnly ? 'true' : 'false');
    updateResultStatus();
    updateClearFiltersBtn();
  }
