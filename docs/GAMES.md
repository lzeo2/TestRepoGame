...
All **29 registered games** from `games.json` (ids 1–47 with gaps — the ids of removed games are not reused). Category comes from the catalog `cat` field; controls and status are based on the actual files in `Games/`.
