class Player extends Ship {
    ...
    controls() {
        // Touch or mouse: drag to move, hold to fire
        if (mouseIsPressed) {
            let dx = mouseX - this.pos.x;
            let dy = mouseY - this.pos.y;
            let dist = Math.hypot(dx, dy);
            if (dist > 2) {
                let step = Math.min(this.speed * 1.6, dist);
                this.vel = createVector(dx / dist * step, dy / dist * step);
            } else {
                this.vel.mult(0);
            }
            this.fire();
    ...
    onDeath() {
        if (typeof showNotice === 'function') {
            showNotice('Ship destroyed. The level restarts.');
        }
        reloadLevel();
    }
    onHitBottom() {
