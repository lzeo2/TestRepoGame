...
var MC = MC || (function() {
    var engine = (function() {
         ...
         */
        function _gameLoop() {
            if (_state === 'run' && !Wave.getWave(_level)) {
                _state = 'won';
            }
            if (_state !== 'run') {
                _drawEndScreen();
        ...
        function countCities() {
            ...
            for (var i = 0; i < _entities.targets.length; i++) {
                if (_entities.targets[i] instanceof Home) {
                    count += 1;
                }
            }
            return count;
        ...
        function removeTarget(target) {
            for (var i = 0; i < _entities.targets.length; i++) {
                if (_entities.targets[i] === target) {
                    _entities.targets.splice(i, 1);
                    break;
                }
            }
            if (countCities() === 0) {
        ...
        function _drawEndScreen() {
            ...
            var title = _state === 'won' ? 'You cleared all 40 waves' : 'Game over';
            _ctx.fillText(title, _width / 2, _height / 2 - 20);
            _ctx.font = '18px sans-serif';
            _ctx.fillText(
                'Score: ' + _score + '. Click or tap to play again.',
                _width / 2,
                _height / 2 + 20
            );
            _ctx.textAlign = 'left';
        ...
        function _moveEntities(entities) {
            ...
            for (var i = 0; i < count; i++) {
                ...
                var intercepted = hasHitRocketExplosion(entities[i]);
                if (intercepted || entities[i].hasHit()) {
                    var target = entities[i].target;
                    // Remove the missile
                    entities.splice(i, 1);
                    count -= 1;
                    ...
                    if (intercepted) {
                        _score += 25;
                    } else if (target instanceof Home) {
                        removeTarget(target);
                    }
                }
