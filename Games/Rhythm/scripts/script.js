...
var showResult = function () {
  ...
  document.querySelector('.result__heading').innerHTML =
    hits.miss > total / 2 ? 'Failed' : 'Cleared';
...
var keyMap = ['s', 'd', 'f', ' ', 'j', 'k', 'l'];

var pressTrack = function (index, pressed) {
  var key = keyMap[index];
  if (pressed && !isHolding[key]) {
    isHolding[key] = true;
    keypress[index].style.display = 'block';
    if (isPlaying && tracks[index].firstChild) {
      judge(index);
    }
  } else if (!pressed && isHolding[key]) {
    isHolding[key] = false;
...
var setupTouch = function () {
  document.querySelectorAll('.key').forEach(function (element, index) {
    element.addEventListener('pointerdown', function (event) {
      event.preventDefault();
      pressTrack(index, true);
    });
    ['pointerup', 'pointerleave', 'pointercancel'].forEach(function (type) {
      element.addEventListener(type, function () {
        pressTrack(index, false);
      });
    });
  });
};
