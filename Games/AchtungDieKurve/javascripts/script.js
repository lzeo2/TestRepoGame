...
(function(Game) {
    var canvasID = 'canvas',
        ...
        // Touch input: the field is split into four vertical zones so a phone can steer
        getTouchControlPlayers = function() {
            var list = [],
                k;

            for (k = 0; k < players.length && list.length < 2; k++) {
                if (players[k].isPlaying) {
                    list.push(players[k].ID);
                }
            }

            return list;
        },
        handleTouchChange = function(event) {
            var controlled = getTouchControlPlayers(),
                next = [0, 0],
                rect = domCanvas.getBoundingClientRect(),
                zone,
                k;

            if (!gameStarted || !controlled.length) {
                return;
            }

            event.preventDefault();

            for (k = 0; k < event.touches.length; k++) {
                zone = Math.floor((event.touches[k].clientX - rect.left) / rect.width * 4);
            ...
            setCurrentDirection(controlled[0], next[0]);

            if (controlled.length > 1) {
                setCurrentDirection(controlled[1], next[1]);
            }
        };
    ...
    if (!domCanvas.getContext) {
        ...
        domCanvas.addEventListener('touchstart', handleTouchChange, false);
        domCanvas.addEventListener('touchmove', handleTouchChange, false);
        domCanvas.addEventListener('touchend', handleTouchChange, false);
        domCanvas.addEventListener('touchcancel', handleTouchChange, false);
