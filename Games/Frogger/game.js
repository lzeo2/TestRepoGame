...
var winGame = function() {
	...
	Game.setBoard(1,new TitleScreen("You win!", 
									"Press the up arrow to play again",
									playGame));
};
...
var Background = function() {
	...
	this.draw = function(ctx) {
		...
		ctx.lineWidth = 3;
		ctx.textAlign = "left";
		var scoreText = "Score: " + FROG_SCORE;
		ctx.strokeText(scoreText, 8, 20);
		ctx.fillText(scoreText, 8, 20);
		var livesText = CURRENT_FROG_LIVES;
		ctx.strokeText(livesText, Game.width/2 - 25, 20);
		ctx.fillText(livesText, Game.width/2 - 25, 20);
		ctx.restore();
	};
};
